...
class PolymarketDataProvider(MarketDataProvider):
    ...
    def get_resolution(self, external_id: str) -> str | None:
        ...
        if prices[1] >= 0.99 and prices[0] <= 0.01:
            return "NO"
        # Обе стороны по 0.50 — событие не состоялось. Так закрываются рынки
        # третьей карты, если серия закончилась 2:0: биржа гасит обе стороны
        # пополам, а не возвращает деньги по цене покупки.
        if abs(prices[0] - 0.5) <= 0.01 and abs(prices[1] - 0.5) <= 0.01:
            return "SPLIT"
