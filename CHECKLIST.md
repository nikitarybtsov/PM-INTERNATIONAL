...
Два транспорта на выбор оператора, как и у Codex: HTTP API по ключу
(`CLAUDE_TRANSPORT=api`) и локальный залогиненный CLI по подписке
(`CLAUDE_TRANSPORT=cli`). Механика запуска CLI общая с Codex — один класс
`cli_base.py`, поэтому условия участников не могут разойтись по недосмотру.

- `app/adapters/participants/claude.py` (API), `claude_cli.py` (CLI),
  общие `cli_base.py`, `llm_base.py`, `prompting.py`
- Тесты: `tests/test_isolation.py::test_prompts_are_identical_except_own_bank`,
  `tests/test_auth_and_codex_cli.py` (7 тестов Claude CLI, включая
  `test_both_cli_participants_get_identical_prompt` — промпты обоих CLI
  сравниваются посимвольно), `tests/test_adapters.py`
...
Открытые позиции переоцениваются по лучшему **bid** — по цене, за которую их можно закрыть
сейчас. Оценка по ask (цене покупки) показывала бы нулевой убыток сразу после сделки и
считала спред бесплатным.

- `app/services/paper_engine.py`, `app/services/book.py`, `app/services/settlement.py`
- Отмена заявки: `paper_engine.cancel_order`
- Тесты: `tests/test_paper_engine.py` (18 тестов, включая
  `test_reproducible_fills_for_same_inputs`, `test_marks_use_best_bid_not_ask`,
  `test_unrealized_pnl_shows_spread_cost_right_after_buy`)
...
**Результат прогона:** `191 passed` (Windows 11, Python 3.12.10, чистый клон).
...
| 12 | Все основные тесты проходят | ✅ | `191 passed` |
...
7. **Открытые позиции оцениваются по лучшему bid** — консервативная mark-to-market оценка:
   позиция стоит столько, за сколько её можно закрыть сейчас. Оценка по ask показывала бы
   нулевой убыток сразу после сделки, то есть считала спред бесплатным (на рынках TI это
   ≈1.4%). На итоговый банк выбор не влияет — после settlement позиции гасятся по факту
   исхода, — только на промежуточный scoreboard.
8. **Автоматизации для Титана нет и не планируется.** Он вводит решения руками через форму;
   планировщик может открыть раунд, но за Титана решение не подаёт. Если он не успевает к
   дедлайну, раунд по умолчанию отменяется (`TITAN_TIMEOUT_POLICY=cancel`), чтобы не портить
   его статистику записанным за него HOLD.
9. **Значения `CODEX_TRANSPORT` и `CLAUDE_TRANSPORT` по умолчанию — `api`, а не `cli`.**
   На боевом сервере планируется `cli` у обоих (расход по подпискам, ключи не нужны), но
   дефолт обязан работать на машине, где ни один CLI не установлен: иначе оба ИИ-участника
   получают `FAILED` в каждом раунде, и demo из коробки не проходит. Боевая конфигурация
   описана в `.env.example` и RUNBOOK.
